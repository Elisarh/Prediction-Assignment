...
```
   ...
   Now, we are going to split the data, 70 percent for training porpouse and the rest for testing:
```{r,echo=TRUE}
...
train_rd$finalModel
```
the error rate is less than 1%, what it´s great and something to expect from a random Forest procedure. So, let´s this method to our test set and see what result we get.
